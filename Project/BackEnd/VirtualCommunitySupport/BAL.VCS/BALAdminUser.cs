...
namespace BAL.VCS
{
    public class BALAdminUser
    {
        ...
        public async Task<ResponseResult> UpdateUserAsync(AddUser user)
        {
            try
            {
                var userCreateResult = await UserUpdateAsync(user);
        ...
        public async Task<string> UserUpdateAsync(AddUser user)
        {
            var userCreate = await _dalAdminUser.UpdateUserAsync(user);
            return userCreate;
        }
